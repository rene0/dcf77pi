...
void
fill_thirdparty_buffer(int minute, int bitpos, struct GB_result gbr)
{
	...
	case 0:
		...
		if (bitpos > 1 && bitpos < 8) {
			tpbuf[bitpos - 2] = gbr.bitval == ebv_1 ? 1 : 0;
			/* 2..7 -> 0..5 */
		}
		if (bitpos > 8 && bitpos < 15) {
			tpbuf[bitpos - 3] = gbr.bitval == ebv_1 ? 1 : 0;
			/* 9..14 -> 6..11 */
		}

		/* copy third party type */
		if (bitpos == 1) {
			tpstat = gbr.bitval == ebv_1 ? 2 : 0;
		}
		if (bitpos == 8) {
			if (gbr.bitval == ebv_1) {
				tpstat++;
	...
	case 1:
		...
		if (bitpos > 0 && bitpos < 15) {
			tpbuf[bitpos + 11] = gbr.bitval == ebv_1 ? 1 : 0;
			/* 1..14 -> 12..25 */
		}
		break;
	case 2:
		/* copy third party data */
		if (bitpos > 0 && bitpos < 15) {
			tpbuf[bitpos + 25] = gbr.bitval == ebv_1 ? 1 : 0;
			/* 1..14 -> 26..39 */
		}
		break;
	}
